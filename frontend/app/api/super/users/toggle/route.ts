...
export async function POST(req: Request) {
  ...
  await fetch(`${API_BASE}/api/platform/users/${id}/toggle?isActive=${encodeURIComponent(isActive)}`, {
    method:'POST',
    headers: { Authorization: `Bearer ${decodeURIComponent(token)}` }
  });

  return NextResponse.redirect(new URL('/super/users', req.url), {status: 303});
}
